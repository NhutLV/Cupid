 ...
 */
public class MeetNearbyFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mUserService.getNearbyUsers(UserLogin.getUserLogin(), 1, new Callback<ArrayList<User>>() {
            @Override
        ...
        mScrollListener = new EndlessRecyclerViewScrollListener(mLayoutManager) {
            ...
            public void onLoadMore(int page, int totalItemsCount, RecyclerView view) {
                mUserService.getNearbyUsers(UserLogin.getUserLogin(), page, new Callback<ArrayList<User>>() {
                    @Override
                    public void next(ArrayList<User> result) {
                        mUsers.addAll(result);
                        mAdapter.notifyDataSetChanged();
