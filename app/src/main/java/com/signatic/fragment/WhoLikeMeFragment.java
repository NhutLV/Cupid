 ...
 */
public class WhoLikeMeFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mUserService.getUsersLikeMe(UserLogin.getUserLogin().getID(), 1,UserLogin.getUserLogin().getApiKey(), new Callback<ArrayList<User>>() {
            @Override
        ...
        mScrollListener = new EndlessRecyclerViewScrollListener(mLayoutManager) {
            ...
            public void onLoadMore(int page, int totalItemsCount, RecyclerView view) {
                mUserService.getUsersLikeMe(UserLogin.getUserLogin().getID(), page,UserLogin.getUserLogin().getApiKey(), new Callback<ArrayList<User>>() {
                    @Override
                    public void next(ArrayList<User> result) {
                        mUsers.addAll(result);
                        mAdapter.notifyDataSetChanged();
