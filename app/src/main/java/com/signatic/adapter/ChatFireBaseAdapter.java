...
public class ChatFireBaseAdapter extends RecyclerView.Adapter<ChatFireBaseAdapter.ViewHolder>{
     ...
     * @param id      Device id
     */
    public ChatFireBaseAdapter(List<Chat> dataSet, String id,Context mContext) {
        mId=id;
        mDataSet = dataSet;
    ...
    @Override
    public ChatFireBaseAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v;

        if (viewType == CHAT_RIGHT) {
            v = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.itemchatright, parent, false);
            ViewHolder.date = (TextView) v.findViewById(R.id.date);

        } else {
            v = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.itemchatleft, parent, false);
            ViewHolder.name = (TextView) v.findViewById(R.id.namejob);
        }
